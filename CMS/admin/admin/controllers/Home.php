...
//     }
    ...
    {
        $data['airport_list'] = $this->Home_Model->get_allairport_list();
        //echo '<pre/>';print_r($data['currency_list']);exit;
        $this->load->view('home/airport_list', $data);
    }

    public function manage_airportlist_status()
    {
        if (isset($_POST['airport_id']) && isset($_POST['status'])) {
            $airport_id = $_POST['airport_id'];
            $status = $_POST['status'];
            $update = $this->Home_Model->manage_airportlist_status($airport_id, $status);
            echo $update;
        } else {
            return false;
        }
